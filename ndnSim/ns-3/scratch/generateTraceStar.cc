...
#define LEAF_NODE_START 258
#define MAX_LEAF_NODES 216
...
std::vector<int> getNeighborsFromStarNode(int node){
    std::vector<int> neighbors;
    int minLeafNode = 258;
    int nodesPerRing = 6;
    int maxLeafNode = 474;

    int i = node - minLeafNode;
    int left = i - i%nodesPerRing;
    int right = left +  nodesPerRing + 1;
    if (right >= 474){
	right = 473;
    }
    
    for (int j= left; j < right; j++){
        neighbors.push_back(j+minLeafNode);
    }
    return neighbors;
}
...
int HawkesProcess(int process, double locFactor, std::string content){
    ...
    for (int i =0; i < TIMESTEPSSTARTING; i++){
        ...
        for(int k=0; k<MAX_LEAF_NODES; k++){
        ...
        if (totalNumOneTime > 1){
            totalNumOneTime = totalNumOneTime * .3;
        }      
            //printf("%i\n", totalNumOneTime);
        ...
        for(int j=0; j< totalNumOneTime; j++){
            int randomLocation = rand() % (MAX_LEAF_NODES) + LEAF_NODE_START;
