...
run(opts_t* opts, oml_mps_t* oml_mps)
{
  ...
  do {
    /* The oml_inject_MPNAME() helpers are defined in node_mon_oml.h*/
    oml_inject_node_stats(oml_mps->node_stats, ((uint32_t)val), "foo", 1.0/val, 1.0/val);
...
main(int argc, const char *argv[])
{
  ...
  if((ret = omlc_init("node_mon", &argc, argv, NULL)) < 0) {
    logerror("Could not initialise OML\n");
    return -1;
  }

  /* Parse command line arguments */
  poptContext optCon = poptGetContext(NULL, argc, argv, options, 0); /* options is defined in node_mon_popt.h */
  while ((c = poptGetNextOpt(optCon)) > 0) {}

  /* Initialise measurement points, only if OML is not disabled */
  oml_register_mps(); /* Defined in node_mon_oml.h */
  if(ret == 0 && omlc_start()) {
    logerror("Could not start OML\n");
    return -1;
  }

  run(g_opts, g_oml_mps_node_mon); /* Do some work and injections, see above */
