...
defApplication('MF-Router', 'router') {|app|
	...
	app.path = "./eval/orbit/tutorial/scripts/ARWrapper.sh" 
	# click options
...
defApplication('MF-GNRS', 'gnrs') {|app|
	...
	app.path = "./eval/orbit/tutorial/scripts/GNRSWrapper.sh" 
...
router_GNRS_if_netmask = '10.11.0.10'
router_threads = 4
GNRS_listen_ip = '0.0.0.0'
GNRS_listen_port = "10001"

router_ether_if_ip = Array['10.11.0.1', '10.11.0.2']

# GNRS configuration
GNRS_server_port = "5001"
GNRS_server_ip = '10.11.0.10'

# GNRS configuration file
GNRS_log_file = 'file:./eval/orbit/tutorial/conf/gnrs/log4j.xml'
GNRS_conf_file = './eval/orbit/tutorial/conf/gnrs/server.xml'
GNRS_jar_file = "./gnrs/jserver/target/gnrs-server-1.0.0-SNAPSHOT-jar-with-dependencies.jar"
...
rtr_topo_file = './eval/orbit/tutorial/conf/rtr_topo.tp'
hoststack_conf_file = ['./eval/orbit/tutorial/conf/hoststack1.conf', './eval/orbit/tutorial/conf/hoststack2.conf']

#Click configuration file for MobilityFirst Access Router
click_conf = './router/click/conf/MF_IPAccessMultiRouter.click'
  ...
  #Through the group definition we set up the applications to run
	defGroup("router_#{i}", "topo:router_#{i}") {|node|
		node.addApplication('MF-Router') {|app|
			...
			app.setProperty('GNRS_listen_ip', "10.11.0.#{i}")
