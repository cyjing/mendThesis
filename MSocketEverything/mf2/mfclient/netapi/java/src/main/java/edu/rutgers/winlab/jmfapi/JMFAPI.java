...
public class JMFAPI {
	...
	private native int mfsend(int handle, byte[] data, int size, int dst_GUID, int opts, int na) throws CException;
	 ...
	 *          Usually is due to an error at the API level and in that case contains the errno value
	 */
	public int jmfsend(byte[] data, int size, GUID dst_GUID, int na) throws JMFException{
		if(handle == 0){
			throw new JMFException("Trying to execute an operation without opening the socket session");
		}
		int ret = -1;
		try {
			ret = mfsend(handle, data, size, dst_GUID.getGUID(), 0 , na);
		} catch(CException e) {
            throw new JMFException(e.getMessage(), e.getCause());
		}
		if(ret<0){
	...
	public int jmfsend(byte[] data, int size, GUID dst_GUID, MFFlag opts) throws JMFException{
		...
		int ret = -1;
		try {
			ret = mfsend(handle, data, size, dst_GUID.getGUID(), opts.getValue(), 0);
		} catch(CException e) {
            throw new JMFException(e.getMessage(), e.getCause());
		}
		if(ret<0){
	...
	public int jmfsend(byte[] data, int size, GUID dst_GUID) throws JMFException{
		...
		int ret = -1;
		try {
			ret = mfsend(handle, data, size, dst_GUID.getGUID(), 0, 0);
		} catch(CException e) {
			throw new JMFException(e.getMessage(), e.getCause());
		}
		if(ret<0){
