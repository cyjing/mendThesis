...
fd_edge::FromDevice($edge_dev, PROMISC false, SNIFFER true); //edge port
...
fd_edge 
        ...
        -> edge_cla::Classifier(12/0800 23/05, -) //only IP-encap MF pkts, i.e.,  with IP Protocol ID = 0x05
        -> SetTimestamp 
		-> MF_Learn(IN_PORT 1, ARP_TABLE arp_tbl) // learn src eth, IP, port
        -> Strip(34) // strip eth, IP hdrs; assumes no VLAN tag (tested also with VLAN trunks)
		-> inQ;
