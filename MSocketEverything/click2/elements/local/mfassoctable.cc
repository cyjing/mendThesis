...
MF_AssocTable::MF_AssocTable():logger(MF_Logger::init()),
    _assoc_timeout(ASSOC_TIMEOUT_SEC) {
  _assocTable = new AssocTable(); 

  /*
  std::string key("ASSOC_TIMEOUT_SEC");
  std::string value = MF_Settings::instance()->lookup(key);

  if (!value.empty()) {
    _assoc_timeout = static_cast<uint32_t>(atoll(value.c_str()));
    logger.log(MF_DEBUG, "MF_AssocTable::ctor, settings lookup key: %s, get value in int: %u",
                key.c_str(), _assoc_timeout);
  } 
  */
}
...
int32_t MF_AssocTable::get (uint32_t assoc_guid, Vector<uint32_t> &guid_vec) {
  ...
  if (it == _assocTable->end()) {
    ...
    for (it2 = it.value()->begin(); it2 != it.value()->end(); ++it2) {
      if (MF_Functions::passed_sec(it2.value().ts) < _assoc_timeout) {
  ...
  while (it != _assocTable->end()) {
    ...
    if (it.value()->size() != 1) {
      ...
      HashTable<uint32_t, assoc_table_record_t>::iterator it2 =
          it.value()->begin();
      if (it2.key() == it.key() && 
            MF_Functions::passed_sec(it2.value().ts) < _assoc_timeout) {
