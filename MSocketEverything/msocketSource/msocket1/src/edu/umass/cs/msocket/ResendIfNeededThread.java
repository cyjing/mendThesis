...
public class ResendIfNeededThread implements Runnable
{
  ...
  {
	System.out.println("resendIfNeeded cingo databaseseq:" +cinfo.getDataBaseSeq()+"  dataSendSeq:"+ cinfo.getDataSendSeq() );
  ...
  private void handleMigrationInMultiPath(int tempDataSendSeqNum, SocketInfo Obj) throws IOException
  {
    ...
    if (tempDataSendSeqNum > dataAck)
    {
      ...
      for (int i = 0; i < buf.length/MWrappedOutputStream.WRITE_CHUNK_SIZE; i ++){
    	// FIXME: change it to chunks
          int arrayCopyOffset =0;
          DataMessage dm = new DataMessage(DataMessage.DATA_MESG, dataAck, cinfo.getDataAckSeq(),
        		                           MWrappedOutputStream.WRITE_CHUNK_SIZE, 0, buf, MWrappedOutputStream.WRITE_CHUNK_SIZE*i);
          ...
          Obj.attemptGSTARWrite(writebuf);
      }
