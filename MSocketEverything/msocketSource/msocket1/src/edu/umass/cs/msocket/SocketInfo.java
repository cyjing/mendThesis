...
public class SocketInfo
{
  ...
  // offset, in seq num, of data  from the current chunk
  ...
  private boolean               guidActive                     = true;
  /*
  ...
  // CYJING store guid of socketinfo of other end
  private byte[] otherGUID;
  private byte[] myGUID;
  private int NA;
  private int myNA;
  ...
  public SocketInfo(SocketChannel dataChannel, Socket socket, int SocketIdentifier, byte[] GUID, byte[] myGUID, int na, int myNA)
  {
    this.dataChannel = dataChannel;
    this.socket = socket;
    this.SocketIdentifier = SocketIdentifier;
    ...
    String scheme = "basic";
	GUID srcGUID = null;
	String in = new String(myGUID);
	int dguid = Integer.parseInt(in.trim());
	srcGUID = new GUID(dguid);

	System.out.println("MY SRCGUID: " + dguid);
	receiver = new JMFAPI();
	// TODO, close receiver at the end when connectionInfo closes
   ...
   * @param socket
   */
  public synchronized void setSocketInfo(SocketChannel dataChannel, Socket socket, int na)
  {
   ...
   * @param socket
   */
  public synchronized void setSocketInfo(SocketChannel dataChannel, Socket socket, byte[] GUID, int na)
  {
	  setSocketInfo( dataChannel,  socket, na);
	  this.otherGUID = GUID;
  }
  ...
  public int attemptGSTARRead(byte[] byteArray) throws IOException
  {
	  int ret = 0;
<<<<<<< HEAD
	  GUID srcGUID = null;
      String in = new String(otherGUID);
	  int dguid = Integer.parseInt(in.trim());
	  srcGUID = new GUID(dguid);
	  
	  try{
		// CYJING TODO, if byteArray is too large, need to do a while loop
		  ret = receiver.jmfrecv(srcGUID, byteArray, byteArray.length);
		  if (ret > 0){
			  System.out.println("Arrays.toString(byteArray)" + " ___ from client of size: " + ret);
=======
	  if (guidActive){
		  //System.out.println(new String(otherGUID));
		  GUID srcGUID = null;
	      String in = new String(otherGUID);
		  int dguid = Integer.parseInt(in.trim());
		  srcGUID = new GUID(dguid);
		  ...
		  try{
			  ...
			  ret = receiver.jmfrecv(null, byteArray, byteArray.length);
			  if (ret > 0){
				  System.out.println(System.currentTimeMillis() + "  from client of size: " + ret);
			  }
		  } catch (JMFException e) {
			// TODO Auto-generated catch block
...
>>>>>>> d6a06c12b1fbb65c877c26719f4cbf5e8a9ba955
  ...
  public synchronized int attemptGSTARWrite(byte[] input) throws IOException
  {
	 int ret = 0;
	 ...
	 if (guidActive){
		...
		byte[] byteNA = new byte[] {(byte)(NA >>> 24),(byte)(NA >>> 16),(byte)(NA >>> 8),(byte)NA};
		if (input.length > DataMessage.HEADER_SIZE + 4){
			input[DataMessage.HEADER_SIZE] = byteNA[0];
			input[DataMessage.HEADER_SIZE + 1] = byteNA[1];
			input[DataMessage.HEADER_SIZE + 2] = byteNA[2];
			input[DataMessage.HEADER_SIZE + 3] = byteNA[3];
		}
		
		try {
			ret = receiver.jmfsend(input, input.length, dstGUID, NA);
			//System.out.println("Attempted single transfer of size:" + input.length);
		} catch (JMFException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		System.out.println("Attempt to GSTAR write: " + dguid + "  with ret: " + ret + " NA:" + NA + " len:" + input.length);
	 }
	return ret;
  }
