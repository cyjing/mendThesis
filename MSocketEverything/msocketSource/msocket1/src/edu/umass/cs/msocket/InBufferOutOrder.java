...
public class InBufferOutOrder
{
  ...
  public synchronized void setDataAck(int s){
	  dataReadSeq = s;
  }

  public synchronized void setByteRecvInInbuffer(int s){
	  byteRecvInInbuffer = s;
  }
  
  public synchronized long getByteRecvInInbuffer(){
	  return byteRecvInInbuffer;
  }
  ...
  /**
	...
	public synchronized boolean isDataInOrder(int chunckStartSeq, int chunkLength) {
		System.out.println("dataReadSeq: " + dataReadSeq + " chunckStartSeq:" + chunckStartSeq
			...
			int chunkLen, byte[] appBuffer, int offset, int appLen) 
	{
		//#if(chunkLen > 0)
		//{
		System.out.println("copyOrderedDataToAppBuffer: "+" startSeqNum "+startSeqNum+" chunkLen "+chunkLen+
