...
public class MServerSocket extends ServerSocket
{
      ...
      throws IOException
  {
	...
	// CYJING, quick hack for running on orbit labs
  ...
  private void initializeServerSocket(String serverName, ProxySelectionPolicy proxyPolicy
		  , SocketAddress endpoint, int backlog) throws IOException
	{
	    ...
	    //CYJING TODO for testing purposes, set server to be GUID 101
	    String tmpGUIDTest = "10001";
	    System.arraycopy(tmpGUIDTest.getBytes(), 0, myGUID, 20-tmpGUIDTest.length(), tmpGUIDTest.length());
		//System.arraycopy(serverAlias.getBytes(), 0, myGUID, 20-serverAlias.length(), serverAlias.length());
	    ...
	    if (endpoint == null)
	    {
	      ...
	      //CYJING orbit ip hack
	      //InetAddress serverAddr = CommonMethods.returnLocalPublicIP();
	      InetAddress serverAddr=InetAddress.getLocalHost();
	      System.out.println("IP of my system is := "+serverAddr.getHostAddress());
	    ...
	    // We have no proxy
	    {
	      // application has opened it with server name
	      if(!serverAlias.equals(""))
	      {
		      try
		      {
  ...
  private class Handler implements Runnable
  {
    ...
    public void run()
    {
    	if (!proxySelection.hasAvailableProxies())
    	{
    		...
    		// CYJING -- can do later, figure out how to deal with initial 
    		// socket connection issues
    		
    		
    		System.out.println("new connection accepted by socket channel");
    		InternalMSocket ms = null;
    		try
    		{
    			System.out.println("myGUID: " + myGUID.toString());
    			ms = new InternalMSocket(connectionSocketChannel, controller, null, myGUID);
    		}
    		catch (IOException e)
    		{
    			e.printStackTrace();
