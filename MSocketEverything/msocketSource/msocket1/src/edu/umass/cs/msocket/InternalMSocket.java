...
public class InternalMSocket extends MSocket
{
  ...
  protected InternalMSocket(SocketChannel newChannel, MServerSocketController serverController,
	      SetupControlMessage clientSCM, byte[] GUID) throws IOException
  {
      //before it accepts anything, no other GUID attached
      setGUID(GUID, GUID);
      this.serverController = serverController;
      // necessary to set blocking mode to non-blocking
      // other wise due to race condition it blocks as by
      ...
      throws IOException
  {
    ...
    System.out.println("The flowID is: " + scm.flowID + "controlMessage: " + scm.MesgType + "at address: " + newChannel.getRemoteAddress());
    ...
    switch (scm.MesgType)
    {
      ...
      {
        log.debug("NEW_CON_REQ recv at server from " + newChannel.socket().getInetAddress() + " scm.ackSeq "
            + scm.ackSeq + " and GUID of " + scm.GUID + " myGUID: " + new String(myGUID));
        setupControlWrite(serverController.getLocalAddress(), localFlowID, SetupControlMessage.NEW_CON_MESG_REPLY,
            serverController.getLocalPort(), newChannel, scm.SocketId, scm.ProxyId, scm.GUID, 0, 0);
        ...
        System.out.println("myGUID:" + myGUID + "otherGUID: " + scm.GUID);
        SocketInfo sockInfo = new SocketInfo(newChannel, newChannel.socket(), scm.SocketId, scm.GUID, myGUID, scm.NA, 1);
      ...
      case SetupControlMessage.ADD_SOCKET_REQ :
      {
        ...
        // to the server
        cinfo.addSocketHashMap(newChannel, scm.ackSeq, scm.GUID, scm.NA);
        flowID = scm.flowID;
        isNew = false;
        break;
      }
      case SetupControlMessage.ADD_SOCKET_GNRS :  // CYJING add a socket to send based on a NA/GUID pair
      {
    	  cinfo = serverController.getConnectionInfo(flowID);
          if (cinfo == null)
          {
        	log.debug("Something went wrong in adding a sockets GNRS, no cinfo");
        	isNew = false;
          }
          else
          {
      ...
      case SetupControlMessage.ADD_SOCKET_RESET :  // CYJING
      {
          cinfo = serverController.getConnectionInfo(scm.flowID);
          System.out.println("myGUID:" + myGUID + "otherGUID: " + scm.GUID + " otherNA:" + scm.NA);
          ...
          else
          {
            setupControlWrite(serverController.getLocalAddress(), scm.flowID, SetupControlMessage.MIGRATE_SOCKET_REPLY,
                serverController.getLocalPort(), newChannel, scm.SocketId, scm.ProxyId, scm.GUID, 0, scm.NA);
            ...
            // need to set it here
            newChannel.configureBlocking(false);
            SocketInfo sockInfo = new SocketInfo(newChannel, newChannel.socket(), 1, scm.GUID, myGUID, scm.NA, 1);

            // ack seq num overloads the rtt from
            // client to server in newe con message
            sockInfo.setEstimatedRTT(scm.ackSeq);
            sockInfo.setStatus(true); // true means active
            flowID=scm.flowID;
            isNew = false;

            cinfo.resetSocketQueue();
            cinfo.setNextSocketIdentifier(2);
            cinfo.addSocketInfo(1, sockInfo);
            cinfo.inputQueuePutSocketInfo(sockInfo);
            cinfo.outputQueuePutSocketInfo(sockInfo);
            /*
      ...
      case SetupControlMessage.MIGRATE_SOCKET_REQ :
      {
        ...
        else
        {
          ...
          // need to set it here
          newChannel.configureBlocking(false);
          sockObj.setSocketInfo(newChannel, newChannel.socket(), scm.NA);
          sockObj.setStatus(true); // true means active
                                   // again
